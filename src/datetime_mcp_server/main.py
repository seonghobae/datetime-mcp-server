...
def main():
    ...
    if args.transport == "stdio":
        print(f"Starting Datetime MCP Server in STDIO mode...", file=sys.stderr)
        asyncio.run(stdio_main())
    elif args.transport == "http":
        print(f"Starting Datetime MCP Server in HTTP mode on {args.host}:{args.port}...", file=sys.stderr)
        run_http_server(
            host=args.host,
            port=args.port,
