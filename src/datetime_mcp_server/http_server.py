    ...
    try:
        ...
        try:
            if method == "initialize":
                ...
                result = {"messages": [{"role": "user", "content": {"type": "text", "text": msg.content.text}} for msg in prompt_result.messages]}
                ...
                result = {"content": [{"type": "text", "text": content.text} for content in tool_result]}
            else:
...
def create_hypercorn_config(
    ...
    config.workers = workers or max(1, (os.cpu_count() or 1) // 2)  # Conservative worker count
...
def run_http_server(
    ...
    logger.info(f"Workers: {workers or 'auto'}, Reload: {reload}, Log level: {log_level}")
