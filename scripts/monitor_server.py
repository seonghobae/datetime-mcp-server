...
class ServerMonitor:
            ...
            for line in iter(stream.readline, ""):
                if line.strip():
                    ...
                    if any(
                        ...
                        print(
                            f"[{datetime.now()}] 🚨 ERROR DETECTED in {name}: {line.strip()}"
                        )
    ...
    async def run_monitoring(self):
        ...
        print(
            f"[{datetime.now()}] 🔍 Starting server monitoring for {self.monitoring_duration} seconds..."
        )
        ...
        try:
            while (
                ...
                and (time.time() - self.start_time) < self.monitoring_duration
            ):
                metric, process_alive = self.monitor_process(pid)

                if metric:
                    print(
                        f"[{datetime.now()}] 📊 Memory: {metric['memory_rss_mb']:.1f}MB "
                    ...
                    if len(self.metrics) > 10:
                        ...
                        if max(recent_memory) - min(recent_memory) > 10:  # 10MB growth
                            print(
                                f"[{datetime.now()}] ⚠️ MEMORY GROWTH DETECTED: "
    ...
    def analyze_metrics(self):
        ...
        fd_values = [
            m["num_file_descriptors"]
            for m in self.metrics
            if m["num_file_descriptors"] > 0
        ]
        if fd_values:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monitor datetime MCP server stability"
    )
    parser.add_argument(
        "--duration",
        type=int,
