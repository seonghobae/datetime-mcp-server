...
def analyze_performance_requirements(benchmarks: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for benchmark in benchmarks:
        ...
        operation_data = {
            ...
            "performance_ratio": p95_threshold_us / p95_estimate_us if p95_estimate_us > 0 else float('inf')
        }
    ...
    if operations:
        ...
        analysis["performance_summary"] = {
            ...
            "passing_operations": sum(1 for op in operations if op["meets_requirement"]),
            "fastest_mean_ms": operations[0]["mean_us"] / 1000,
            "slowest_mean_ms": operations[-1]["mean_us"] / 1000,
            "average_mean_ms": sum(op["mean_us"] for op in operations) / len(operations) / 1000,
            "average_performance_ratio": sum(op["performance_ratio"] for op in operations) / len(operations)
        }
...
<body>
    <div class="container">
        ...
        <div class="status {'pass' if analysis['p95_requirement_met'] else 'fail'}">
            {'✅ PASSED' if analysis['p95_requirement_met'] else '❌ FAILED'}: p95 ≤ 50ms Requirement
        </div>
        
        <h3>📊 Performance Summary</h3>
        <div style="text-align: center;">
            <div class="metric">
                <div class="metric-value">{analysis['performance_summary']['total_operations']}</div>
                <div class="metric-label">Total Operations</div>
            </div>
            <div class="metric">
                <div class="metric-value">{analysis['performance_summary']['passing_operations']}</div>
                <div class="metric-label">Passing Tests</div>
            </div>
            <div class="metric">
                <div class="metric-value">{analysis['performance_summary']['fastest_mean_ms']:.3f}ms</div>
                <div class="metric-label">Fastest Operation</div>
            </div>
            <div class="metric">
                <div class="metric-value">{analysis['performance_summary']['average_mean_ms']:.3f}ms</div>
                <div class="metric-label">Average Response Time</div>
            </div>
            <div class="metric">
                <div class="metric-value">{analysis['performance_summary']['average_performance_ratio']:.1f}x</div>
        ...
        <ul>
            <li><strong>Fastest Operation:</strong> {analysis['fastest_operation']['name']} - {analysis['fastest_operation']['mean_us']/1000:.3f}ms (avg)</li>
            <li><strong>Slowest Operation:</strong> {analysis['slowest_operation']['name']} - {analysis['slowest_operation']['mean_us']/1000:.3f}ms (avg)</li>
            <li><strong>Overall Average:</strong> {analysis['performance_summary']['average_mean_ms']:.3f}ms</li>
            <li><strong>Performance Achievement:</strong> Operations are <strong>{analysis['performance_summary']['average_performance_ratio']:.1f}x faster</strong> than the 50ms requirement!</li>
            <li><strong>Requirement Compliance:</strong> {analysis['performance_summary']['passing_operations']}/{analysis['performance_summary']['total_operations']} operations meet p95 ≤ 50ms</li>
        </ul>
        ...
        ''' if analysis['failing_tests'] else ''}
        ...
        <ul>
            <li><strong>p95 Response Time:</strong> {'✅ PASSED' if analysis['p95_requirement_met'] else '❌ FAILED'} (≤ 50ms)</li>
...
def main():
    ...
    parser.add_argument(
        "--output-text",
        help="Output text report file (optional)"
    )
    parser.add_argument(
        "--print-summary",
        action="store_true",
